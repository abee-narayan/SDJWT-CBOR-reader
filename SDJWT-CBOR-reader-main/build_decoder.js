...
const js = \/* CredentialDecoder – SD-JWT VC & mDOC (ISO 18013-5 CBOR) */
...
function decodeSDJWT(raw){
  ...
  parts.slice(1).forEach(d=>{
    try{const obj=JSON.parse(parseUtf8Base64Url(d));if(Array.isArray(obj)&&obj.length>=3)c[obj[1]]=obj[2];}catch(e){}
  });
...
const CM = {
  ...
  RegionalAddress:{l:'Full Address (Local)',t:'address'},regional_address:{l:'Full Address (Local)',t:'address'},
  building:{l:'Building',t:'address'},Building:{l:'Building',t:'address'},
  LocalBuilding:{l:'Building (Local)',t:'address'},local_building:{l:'Building (Local)',t:'address'},
...
fs.writeFileSync('decoder.js', js, 'utf-8');
