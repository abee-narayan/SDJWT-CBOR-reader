...
function decodeSDJWT(raw){
  ...
  parts.slice(1).forEach(d=>{
    try{const obj=JSON.parse(b64urlDecode(d));if(Array.isArray(obj)&&obj.length>=3)c[obj[1]]=obj[2];}catch(e){}
  });
    ...
    Object.keys(ns).forEach(nsK=>{
      ns[nsK].forEach(ib=>{
        try{
          ...
          if(k.toLowerCase().includes('image')&&ArrayBuffer.isView(v))v='data:image/jpeg;base64,'+uint8ToB64(v.buffer||v);
          c[k]=v;
        }catch(e){}
      });
    });
